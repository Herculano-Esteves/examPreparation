"""
debug_tools/test_dual_slider.py
-------------------------------
Testa a interação com os sliders duplos de Nº de Questões e Percentagem de Acerto.
Simula o clique e arrasto no slider MIN e no slider MAX, aferindo se os valores e eventos disparam corretamente.
"""
...
profile_dir = os.path.join(os.environ["TEMP"], "brave_slider_test_" + str(int(time.time())))
...
try:
    ...
    # Inspect slider elements, bounding boxes, z-index, pointer-events, and values
    inspect_expr = """
    (() => {
        const qMin = document.getElementById('slider-questions-min');
        const qMax = document.getElementById('slider-questions-max');
        const sMin = document.getElementById('slider-score-min');
        const sMax = document.getElementById('slider-score-max');
    ...
    """
    ws_send(s, {"id": 20, "method": "Runtime.evaluate", "params": {"expression": inspect_expr, "returnByValue": True}})
    
    # Simulate changing max slider value via event
    change_max_expr = """
    (() => {
        const qMax = document.getElementById('slider-questions-max');
        if (!qMax) return 'NO_QMAX';
        qMax.value = '10';
        qMax.dispatchEvent(new Event('input', { bubbles: true }));
        return {
            qMaxValue: qMax.value,
            inputFieldVal: document.getElementById('filter-questions-max')?.value,
            visibleExams: document.querySelectorAll('#exams-grid .exam-list-row').length
        };
    })()
    """
    ws_send(s, {"id": 30, "method": "Runtime.evaluate", "params": {"expression": change_max_expr, "returnByValue": True}})
