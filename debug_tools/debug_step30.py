"""
debug_tools/debug_step30.py
--------------------------
Inspeciona exatamente o que o State contém antes e depois do input no slider MIN.
"""
...
profile_dir = os.path.join(os.environ["TEMP"], "brave_dbg30_" + str(int(time.time())))
...
try:
    ...
    # Step 2: Check state after setting max to 20
    ws_send(s, {
        "id": 25,
        "method": "Runtime.evaluate",
        "params": {
    ...
    # Step 4: Check state after setting min to 15
    ws_send(s, {
        "id": 35,
        "method": "Runtime.evaluate",
        "params": {
    ...
    ws_send(s, {
        ...
        "params": {
            "expression": """
            (() => {
                const btn = document.getElementById('btn-reset-global-filters');
                btn?.click();
    ...
    # Step 6: Check state after reset
    ws_send(s, {
        "id": 45,
        "method": "Runtime.evaluate",
        "params": {
