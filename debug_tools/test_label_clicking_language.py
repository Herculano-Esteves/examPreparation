"""
debug_tools/test_label_clicking_language.py
-------------------------------------------
Testa se ao clicar em qualquer parte da linha (no texto, bandeira, contador ou caixa),
o filtro desseleciona/seleciona a língua normalmente, exatamente como as outras opções.
"""
...
profile_dir = os.path.join(os.environ["TEMP"], "brave_label_click_" + str(int(time.time())))
...
try:
    ...
    # Step 1: Click the Portuguese label text (span.floating-type-label) to deselect PT
    ws_send(s, {
        "id": 20,
        "method": "Runtime.evaluate",
        "params": {
            "expression": """
            new Promise(resolve => {
                const ptLabelSpan = document.querySelector('.floating-checkbox-item[data-lang="pt"] .floating-type-label');
                ptLabelSpan?.click();
                ...
                setTimeout(() => {
                    ...
                    const headerText = document.getElementById('floating-filter-count-text')?.textContent?.trim();

                    resolve(JSON.stringify({
                        stage: 'AFTER_CLICK_PT_TEXT',
                        ptChecked,
                        enChecked,
                        visibleExams,
                        headerText
    ...
    # Step 2: Click the Portuguese label text again to re-select PT
    ws_send(s, {
        "id": 30,
        "method": "Runtime.evaluate",
        "params": {
            ...
            new Promise(resolve => {
                ...
                setTimeout(() => {
                    ...
                    resolve(JSON.stringify({
                        stage: 'AFTER_CLICK_PT_TEXT_AGAIN',
                        ptChecked,
                        enChecked,
                        visibleExams,
                        headerText
