...
profile_dir = os.path.join(os.environ["TEMP"], "brave_rules_" + str(int(time.time())))
...
try:
    with urllib.request.urlopen("http://localhost:9226/json") as resp:
    ...
    res = send_cdp("Runtime.evaluate", {
        "expression": """
        (() => {
            const btn = document.getElementById('btn-back-cadeiras');
    ...
    print("Computed btn-back-cadeiras:", res.get("result", {}).get("result", {}).get("value"))
